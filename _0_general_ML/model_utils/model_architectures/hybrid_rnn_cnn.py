...
def hybrid_rnn_cnn(
    ...
    **kwargs
    ...
    if default_model_configuration['weight_decay'] == 0:
        ...
        kernel_regularizer = tf.keras.regularizers.l2(default_model_configuration['weight_decay'])
    ...
    if default_model_configuration['embedding_matrix']:
        ...
        model.add(
            tf.keras.layers.Embedding(
                data.size_of_vocabulary, 
                default_model_configuration['embedding_depth'], 
                input_length=data.max_len
            )
        )
    ...
    model.add(
        tf.keras.layers.Reshape(
            target_shape=(data.max_len, default_model_configuration['embedding_depth'], -1)
        )
    )
    ...
    model.add(tf.keras.layers.LSTM(32, kernel_regularizer=kernel_regularizer))
    
    model.add(tf.keras.layers.Flatten())
    
    model.add(
        tf.keras.layers.Dense(
            data.get_output_shape()[0]), kernel_regularizer=kernel_regularizer)
